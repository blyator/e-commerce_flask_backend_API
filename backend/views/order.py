            ...
            order_dict.update(
                {
                    ...
                    "total_price": (
                        float(order.total_price) if order.total_price else 0.0
                    ),
...
def get_order_invoice(order_id):
    ...
    try:
        ...
        invoice_data = {
            ...
            "subtotal": float(
                sum(item.price_at_order * item.quantity for item in order.order_items)
            ),
...
def update_order_status(order_id):
    ...
    try:
        ...
        return jsonify({"message": "Order status updated", "order": order_dict}), 200
