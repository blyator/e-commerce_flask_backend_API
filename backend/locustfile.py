...
class BaseUser(HttpUser):
    ...
    def login(self):
        ...
        credentials = {"email": "user@demo.com", "password": "demo1234"}

        with self.client.post(
            "/shop-api/login", json=credentials, catch_response=True
        ) as response:
...
class BrowsingUser(BaseUser):
    ...
    def view_products_with_filter(self):
        ...
        params = {
            ...
            "sort": random.choice(
                ["newest", "price-low", "price-high", "rating", "name"]
            ),
        }
        self.client.get(
            "/shop-api/products/",
    ...
    def view_categories(self):
        ...
        self.client.get(
            "/shop-api/products/categories", headers=self._get_auth_headers()
        )
...
class ShoppingUser(BaseUser):
    ...
    def add_to_cart(self):
        ...
        response = self.client.get(
            "/shop-api/products/", headers=self._get_auth_headers()
        )
    ...
    def checkout(self):
        ...
        cart_response = self.client.get(
            "/shop-api/cart", headers=self._get_auth_headers()
        )
                ...
                if cart:  # Only checkout if cart has items
                    ...
                    with self.client.post(
                        ...
                        if resp.status_code == 201:
                            ...
                            self.client.delete(
                                "/shop-api/cart/clear", headers=self._get_auth_headers()
                            )
                        else:
